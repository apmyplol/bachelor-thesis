...
In the beginning of the 19th century the physicist and philosopher N. R. Campbell heavily criticized psychology and the social sciences and argued that, in contrast to physics, it is impossible to construct measurement in these areas \parencite{campbellPhysicsElements19201957}.
...
This means, we search for a function $\varphi$ that maps each sound $\ps{a}$ to a real number $\varphi(\ps{a})$, which we can call the ``loudness'' of sound $\ps{a}$. Naturally, we want the function $\varphi$ to preserve the ordering by $\succsim$. If sound $\ps{a}$ is perceived louder than $\ps{b}$ (i.e., $\ps{a} \succsim \ps{b}$), then we want the corresponding numbers to be in the same order, which is, $\varphi(\ps{a}) \geq \varphi(\ps{b})$. A function satisfying these properties is called a \emph{homomorphism}.
