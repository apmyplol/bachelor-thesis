...
\begin{defi}[Additive-Difference Model] 
    ...
    \item \emph{Decomposability}: The distance between stimuli is a function of componentwise contributions.\\ \index{Decomposability}
        ...
        \item \emph{Intradimensional subtractivity}: Each componentwise contribution is the absolute value of an appropriate scale difference.\\ \index{Intradimensional subtractivity}
		      ...
		      \end{equation}
		      where $F$ is as in \cref{def-decomp} and $\psi_i$ is replaced with $|\varphi_i(a_i) - \varphi_i(b_i)|$ for some real-valued function $\varphi_i$ defined on $A_i$ for $i = 1, ..., n$.
      ...
      \begin{equation}\label{def-add-diff-model} \index{Additive-difference model} \index{AD model}
