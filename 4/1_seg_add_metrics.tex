\chapter{Segmental Additivity}

\section{Segmentally Additive Metrics}
...
\begin{defi}[Curves and Segments] 
	...
	\begin{enumerate}
        ...
        \item For $a \leq x \leq b$ we denote the length of the sub-curve $\curve(t),~ a \leq t \leq x$ from $a$ to $x$ by $\sublen$. This is also called the \emph{arc length} of the curve. \label{sublen} \index{Parametric Curve!arc-length}
	\end{enumerate}
\end{defi}
...
\begin{defi}[Metric Space with Additive Segments] \index{Metric Space!with additive segments}
