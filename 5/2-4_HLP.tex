...
\begin{defi}[Quasi Arithmetic Means and Homogeneity] \index{Quasi-arithmetic mean} \index{Quasi-arithmetic mean!homogeneous} %\gls{quasi-arith-mean}
	...
	Then the \emph{quasi-arithmetic $\phi$-mean of $\ve{x}$ with weights $\ve{w}$} is defined as 
...
\begin{defi}[Power mean] \index{Power mean} %\gls{powermean}
	For $n \in \mathbb{N}, \ve{x}, \ve{w} \in \mathbb{R}_{\geq 0}^n$ and $ p > 0$ we define the \emph{power means} $\meanp{\ve{x}}$ as
	$$ \meanp{\ve{x}} = \left ( \frac{1}{\sum_{i=1}^{n} w_i} \sum_{i=1}^{n}w_i x_i^p \right)^{1/p}. $$
	For $p=0$ we can define $\meanp[0]{\ve{x}}$ as the \emph{geometric mean}
	$$
\meanp[0]{\ve{x}} = \left ( \prod_{i=1}^n x_i^{w_i} \right )^{1 / \sum_{i=1}^{n}w_i}
	$$
	since it is obtained as a limit case for $p \rightarrow 0$ \parencite[p. 176]{handbookMeans}.
\end{defi}
...
Now we continue with a characterization of equal means.
...
\begin{proof}
  ...
  \begin{proofpart}[$\Rightarrow$]
    ...
    Putting $\phi(x) = \chi ( \psi^{-1} (x))$ and $x_i = \psi (a_i)$ for $i=1, 2$ yields
    \begin{equation*}
    ...
    By \textcite[p. 46]{aczel1966lectures}, the general continuous solution is $\psi(x) = \alpha x + \beta$, hence $\chi = \alpha \psi + \beta$.
  \end{proofpart}
...
Now we proceed with the main theorem of this subsection.
...
\begin{proof}
  ...
  \end{align*}
  by setting $\psi(x) = \phi(t x)$. From this, it follows that $\psi^{-1}(x) = t^{-1} \cdot \phi(x)$\footnote{For this, let $\psi^{-1}(x) = y$. Applying $\psi$ to both sides, and using its definition gives $x = \phi(ty)$. Applying $\phi^{-1}$ and dividing by $t$ results in $t^{-1} \phi^{-1}(x) = y$, which in turn is equal to $\psi^{-1}(x)$ by the first equation.}, which in turn justifies the above equation.
  ...
  \end{equation*}
  Plugging in $x=1$ and using $\phi(1) = 0$ yields
  \begin{equation*}
  \phi(t) = \psi(1) = \beta(t).
  \end{equation*}
  ...
  Since this must hold for all $x$ and $y$, the expressions must reduce to a constant $c$. This can be seen by setting, for example, $x=1$ and observing that the right-hand side must equal the left-hand side for all $y$, and therefore must be constant. The same argument can be applied in reverse.
  ...
  $$  \phi(xy) = \phi(x) + \phi(y), $$
  of which the most general and for $x>0$ continuous solution is $\phi(x) = \alpha \log(x)$ for some constant $\alpha \in \mathbb{R}$ \parencites[p. 39]{aczel1966lectures}[p. 110-111]{cauchy1821}.

  If $c \neq 0$, we set $f(x) = c \phi(x) + 1$ and \Cref{proof-quasi-eq1} reduces to the multiplicative Cauchy functional equation
  $$ f(xy) = f(x)f(y), $$
  whose general solution is $f(x) = x^r$ \parencites[p. 39]{aczel1966lectures}[p. 111-112]{cauchy1821}. Substituting back yields $\phi(x) = \frac{x^r -1}{c}$.
\end{proof}
