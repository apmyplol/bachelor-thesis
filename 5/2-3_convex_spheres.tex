...
In this subsection we will establish the convexity of balls which is needed to conclude the proof of \cref{thm-homogeneity}. 
Note that everything is still assumed as stated in the beginning of \cref{sect-homogeneity-theorem}, particularly that all topological terms refer to the natural topology of Euclidean space.
...
\end{lem}
First, we state some definitions and results on basic convex geometry, and then we prove the lemma at the end of this subsection.

We begin by introducing the notions of convex combinations, the convex hull and extreme points.
Recall that a set is convex if for any two points from that set, the line segment connecting them is entirely contained within the set. By increasing the number of points, we get what is called the convex combination of these points, which generalizes from a line to a convex polygon. A convex combination is similar to a linear combination, but with the additional constraint that the coefficients must be positive and add up to $1$. The convex hull of a set is, like any other hull, the smallest convex set that contains it.
...
For compact and convex sets, the extreme points are sufficient to ``build up'' the entire set.
In particular, this means that for such a set, every point can be expressed as a convex combination of its extreme points, as stated in the next theorem.
...
\begin{proof}
  $\metric : S' \times S' \to \mathbb{R}_{\geq 0}$ is continuous with $S' \subseteq \mathbb{R}^n$ and therefore $\ve{y} \mapsto \metro{\ve{y}} = \metr{\ve{0}}{\ve{y}}$ is continuous.
	Consider the closed interval $U' = [0, \dis] \subset \mathbb{R}_{\geq 0}$. Then
	\begin{align*}
    \metric^{-1}(U') & = \{\ve{y} \in S' : \metro{\ve{y}} \in U'\}     \tag{Preimage definition}   \\
                     & = \{\ve{y} \in S' : \metr{\ve{0}}{\ve{y}} \leq \dis\} \tag{Definition of $\metric$} \\
                     & = \cball{\dis}.
	\end{align*}
...
\begin{proof}
  ...
  $$\frac{1}{m}\ve{y} = \sum_{k=1}^N \lambda_k \x \quad \text{with } \sum_{k=1}^N \lambda_k = 1, \quad \lambda_k > 0,~ \x \in \CO[\frac{\dis}{m}] \text{ and } N \in \mathbb{N} $$
	Because $\CO[\frac{\dis}{m}] = \co(\cball{\frac{\dis}{m}})$ holds, we can write any element from $\CO[\frac{\dis}{m}]$ as a convex combination of elements from $\cball{\frac{\dis}{m}}$, and assume $\x \in \cball{\frac{\dis}{m}}$.
	By translation invariance and the triangle inequality we get for all $k$:
	\begin{align*}
  ...
  This yields, that $\ve{y} = \sum_{k=1}^{N} \lambda_k m \x$ is a strict convex combination (because $\lambda_k m \neq 0$) and therefore $\ve{y}$ is not in $\E$.
\end{proof}
...
\begin{proof}[Proof of \cref{lem-metr-spheres-convex}] \label{proof-metr-spheres-convex}
	...
	By \cref{lemma-delta-balls-closed} $\cball{\dis}$ is closed and by assumption it is also bounded. Therefore, from the Heine-Borel Theorem, it follows that $\cball{\dis}$ is compact.	By definition and \cref{thm-compact-hull-compact} it follows, that $\CO$ is compact (and therefore closed, which is the reason for our square bracket notation) and convex. Application of the Minkowski Theorem (\cref{thm-minkowski}) yields
	$$ \CO = \co(\extr ( \CO )). $$
	Further, we have that $\extr(\CO) = \extr(\cball{\dis})$, since convex combinations cannot introduce new extreme points, as these are precisely the points that cannot be expressed as a convex combination.
  Combining this and the statement above yields:
	$$ \cball{\dis} \subset \CO = \co ( \extr (\cball{\dis})). $$
	We proceed by proving that $\co(\extr (\cball{\dis})) \subset \cball{\dis}$, which implies $\cball{\dis} = \CO$ and is equivalent to the convexity of $\cball{\dis}$.
    ...
    \end{cases} \tag{\small{$B$ closed, $\mathbb{Q} \subset \mathbb{R}$ dense \footnote{Density implies, that we can express every real number as a limit of rational numbers. $B$ is closed, and therefore all limits of rational coefficients are included, if the statement holds for rational numbers.}}}                                              \\[1ex]
		 & \centerline{$\Big \Uparrow$}                                                                                               \\[1ex]
	...
	Therefore, the following holds for the convex combination of the $\yy$:
	\begin{align*}
